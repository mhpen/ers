...
<body class="bg-background">
            ...
            <div class="p-6">
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">Monitor your event attendance and check-in statistics</p>
                    </div>
                </div>
                ...
                <!-- Event Details Modal -->
                <div id="eventDetailsModal" class="fixed inset-0 bg-background/80 backdrop-blur-sm hidden z-50">
                    <div class="fixed left-[50%] top-[50%] z-50 grid w-full max-w-3xl translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200 rounded-lg">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-lg font-semibold leading-none tracking-tight" id="modalEventTitle">Event Details</h3>
                                <p class="text-sm text-muted-foreground mt-1" id="modalEventDate"></p>
                            </div>
                            <button onclick="closeEventDetails()" class="text-muted-foreground hover:text-foreground">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            <div class="rounded-lg border bg-card p-4">
                                <h4 class="text-sm font-medium text-muted-foreground">Total Registrations</h4>
                                <div class="mt-2 flex items-center">
                                    <span class="text-2xl font-bold" id="modalRegistrationRate">-</span>
                                    <span class="ml-2 text-sm text-muted-foreground">participants</span>
                                </div>
                            </div>
                            <div class="rounded-lg border bg-card p-4">
                                <h4 class="text-sm font-medium text-muted-foreground">Check-in Rate</h4>
                                <div class="mt-2 flex items-center">
                                    <span class="text-2xl font-bold" id="modalCheckinRate">-</span>
                                    <span class="ml-2 text-sm text-muted-foreground">of registrations</span>
                                </div>
                            </div>
                            <div class="rounded-lg border bg-card p-4">
                                <h4 class="text-sm font-medium text-muted-foreground">Total Attendees</h4>
                                <div class="mt-2 flex items-center">
                                    <span class="text-2xl font-bold" id="modalTotalAttendees">-</span>
                                    <span class="ml-2 text-sm text-muted-foreground">checked in</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="rounded-lg border">
                            <div class="flex items-center justify-between p-4">
                                <h4 class="text-sm font-medium">Attendee List</h4>
    ...
    <script>
        ...
        function fetchAttendanceData() {
            fetch('../../controllers/client/getAttendanceData.php')
        ...
        function showEventDetails(eventId) {
            ...
            fetch(`../../controllers/client/getEventDetails.php?event_id=${eventId}`)
                .then(response => {
                ...
                .then(data => {
                    ...
                    document.getElementById('modalEventTitle').textContent = data.event.title;
                    document.getElementById('modalEventDate').textContent = new Date(data.event.event_date).toLocaleDateString();
                    document.getElementById('modalRegistrationRate').textContent = data.event.total_registrations;
                    document.getElementById('modalCheckinRate').textContent = data.stats.checkin_rate + '%';
                    document.getElementById('modalTotalAttendees').textContent = data.stats.total_attendees;

                    // Populate attendee list
                    const attendeeList = document.getElementById('attendeeListBody');
                    attendeeList.innerHTML = data.attendees.map(attendee => `
                        <tr class="border-b transition-colors hover:bg-muted/50">
                            <td class="p-4 align-middle">${attendee.name}</td>
                            <td class="p-4 align-middle">${attendee.email}</td>
                            <td class="p-4 align-middle">${new Date(attendee.checkin_time).toLocaleString()}</td>
                        </tr>
                    `).join('');
        ...
        function downloadAttendanceCSV() {
            ...
            fetch(`../../controllers/client/downloadAttendance.php?event_id=${currentEventId}`)
                .then(response => response.blob())
                .then(blob => {
                    const url = window.URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = `event-attendance-${currentEventId}.csv`;
